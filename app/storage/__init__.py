...
class Storage: 
    ...
    def get_todo_by_todo_id(id):
        todo_data = db.execute('SELECT * FROM todos WHERE id = ?', (id,)).fetchone()
        if todo_data:
    ...
    def update_todo(todo):
        db.execute('UPDATE todos SET title = ? , description = ? , done = ? WHERE id = ?', (todo.title, todo.description, todo.done, todo.id)).fetchone()
        db.commit()